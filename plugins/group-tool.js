...
async (malvin, mek, m, {
    ...
    try {
        ...
        for (let participant of nonAdminParticipants) {
            try {
                await malvin.groupParticipantsUpdate(from, [participant.id], "remove");
...
async (malvin, mek, m, {
    ...
    try {
        ...
        const allParticipants = groupMetadata.participants;
        const adminParticipants = allParticipants.filter(member => groupAdmins.includes(member.id) && member.id !== malvin.user.id && member.id !== `${botOwner}@s.whatsapp.net`);
...
async (malvin, mek, m, {
    ...
    try {
        ...
        const participantsToRemove = allParticipants.filter(
            participant => participant.id !== malvin.user.id && participant.id !== `${botOwner}@s.whatsapp.net`
        );
